...
struct WifiDisplaySink : public AHandler {
    WifiDisplaySink(
            ...
            const sp<WfdPlayerNotify> &playerNotify = NULL);
...
private:
    ...
    status_t sendTearDown(int32_t sessionID);

    status_t onReceiveM8Response(
    		int32_t sessionID, const sp<ParsedMessage> &msg);
	...
	char* GetConfiguration(const char* input);

    AString mWfd_presentation_url;

    sp<WfdPlayerNotify> mPlayerNotify;
