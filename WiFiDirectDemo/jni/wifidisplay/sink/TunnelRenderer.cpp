...
struct TunnelRenderer::PlayerClient : public BnMediaPlayerClient {
    PlayerClient(const sp<WfdPlayerNotify> &playerNotify) : mPlayerNotify(playerNotify) {}

    virtual void notify(int msg, int ext1, int ext2, const Parcel *obj) {
        ALOGI("notify %d, %d, %d", msg, ext1, ext2);
        if (mPlayerNotify != NULL)
        {
        	mPlayerNotify->notify(msg, ext1, ext2);
        }
    }
...
void TunnelRenderer::initPlayer() {
    ...
    mPlayerClient = new PlayerClient(mPlayerNotify);
